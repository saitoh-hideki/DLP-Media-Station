-- DLP Media Station データベースセットアップ
...
-- 既存のポリシーを削除（存在する場合）
DROP POLICY IF EXISTS "Enable read access for all users" ON public.generated_assets;
DROP POLICY IF EXISTS "Enable insert for all users" ON public.generated_assets;
DROP POLICY IF EXISTS "Enable update for all users" ON public.generated_assets;
...
-- 完了メッセージ
SELECT 'DLP Media Station データベースセットアップが完了しました！' as message;